function SpExCue_runPilot2(ID)
%SpExCue_runPilot2 - Experimental routine for pilot experiment of SpExCue
%project testing the effect of spectral magnitude compression of HRTFs on 
%sound externalization 
%
% Changes from Pilot to Pilot2: M may remain the same.
...
SPL = 70; % SPL in dB
SPLrove = 00; % roving of SPL in dB
rdepth = 0;
rdensity = 0.25;
...
flags.sourceSignal = {'continuousNoise'}; %,'AMnoiseBurst','noiseBurst','speech'
...
%% Save results
fn = fullfile('Results',['SpExCue_pilot2Result_' subj.ID]);
if flags.do_monaural
  fn = [fn '_mon'];
end
